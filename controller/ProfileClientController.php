<?php
  ...
  function show() {
    ...
    $results->bindValue(':id', $_SESSION['session-client']['id']);
    $results->execute();
  ...
  function update($request) {
    ...
    if (!isset($request->password)) {
      $sql = "UPDATE clients SET name = :name, email = :email, updated_at = :updated_at WHERE id = :id";
    } else {
      $sql = "UPDATE clients SET name = :name, email = :email, password = :password ,updated_at = :updated_at WHERE id = :id";
    }
    ...
    $user = [
      'id' => intval($_SESSION['session-client']['id']),
      'name' => $request->name,
      'email' => $request->email,
    ];
